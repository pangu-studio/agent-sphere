...
#
# Usage:
#   ./make-rootfs-qwenpaw.sh [output.qcow2]
#   ./make-rootfs-qwenpaw.sh --force [output.qcow2]
#   ./make-rootfs-qwenpaw.sh --from-step N
#   ./make-rootfs-qwenpaw.sh --list-steps
#   ./make-rootfs-qwenpaw.sh --status
...
CHECKPOINT_DIR="$CACHE_DIR/checkpoints-qwenpaw-arm64"
...
do_convert_qcow2() {
    if [ -z "$OUTPUT" ]; then
        if [ -n "$QWENPAW_VERSION" ]; then
            OUTPUT="$OUTPUT_DIR/rootfs-qwenpaw-${QWENPAW_VERSION}-arm64.qcow2"
        else
            OUTPUT="$OUTPUT_DIR/rootfs-qwenpaw-arm64.qcow2"
        fi
    fi
